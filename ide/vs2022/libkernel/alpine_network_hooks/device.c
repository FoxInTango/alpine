...
#pragma NDIS_INIT_FUNCTION(alpinenetworkhooksRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
alpinenetworkhooksRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>alpinenetworkhooksRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = alpinenetworkhooksDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = alpinenetworkhooksDispatch;
    DispatchTable[IRP_MJ_CLOSE] = alpinenetworkhooksDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = alpinenetworkhooksDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==alpinenetworkhooksRegisterDevice: %x\n", Status);
...
_IRQL_requires_max_(PASSIVE_LEVEL)
VOID
alpinenetworkhooksDeregisterDevice(
    VOID
    )
...
_Use_decl_annotations_                
NTSTATUS
alpinenetworkhooksDeviceIoControl(
    PDEVICE_OBJECT        DeviceObject,
    PIRP                  Irp
    )
{
